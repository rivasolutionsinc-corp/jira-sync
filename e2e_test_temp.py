...
class E2ETestRunner:
    ...
    def step_1_create_jira_issue(self):
        ...
        try:
            # Use pre-created test issue if JIRA_TOKEN not available
            if not os.getenv('JIRA_TOKEN'):
                self.log("⚠ JIRA_TOKEN not set, using pre-created test issue")
                self.jira_issue_key = "CLOUD-1917"
                self.log(f"✓ Using test Jira issue: {self.jira_issue_key}")
...
EOF
            ...
            if returncode == 0 and stdout:
                self.jira_issue_key = stdout.strip()
